...
carregado no **Android** por meio de uma fachada UniFFI na carteira. Como os três rodam
exatamente o mesmo código, a compatibilidade byte a byte é por construção; um oráculo de
conformidade guarda a fronteira FFI da carteira em vez de uma segunda implementação.
...
| **`ssi-core`** | `crates/ssi-core` | O motor SSI compartilhado — JWS (EdDSA + ES256), SD-JWT, DCQL, OID4VP, status lists, `did:web`. Roda nativamente, como **WebAssembly** (o verificador) e no **Android** via a fachada UniFFI `crates/wallet-ffi` (a carteira). |
...
| **Carteira** | `wallet/` (Kotlin/Android) | O **titular**: recebe credenciais sobre OID4VCI, as armazena e as apresenta sobre OID4VP. Roda o motor `ssi-core` compartilhado diretamente pela fachada UniFFI `crates/wallet-ffi` — sem reimplementação de criptografia em Kotlin; o oráculo em `crates/wallet-core` exercita a fronteira FFI. Veja [`wallet/README.md`](wallet/README.pt-BR.md). |
...
**Android SDK** (+ um emulador) e — como o motor da carteira é o `ssi-core` em Rust
compilado para o celular — do **NDK**, do `cargo-ndk` e dos alvos Android do rustup. A
camada Kotlin `:ssi` e seus testes rodam apenas com **JDK + `cargo`** (para compilar a
biblioteca FFI do host) — sem necessidade da stack Android.
...
`TEST_DATABASE_URL`, executa toda a suíte Rust e o clippy, compila a biblioteca FFI do host
(`wallet-ffi-host`) e reexecuta o oráculo de conformidade Kotlin.

Limite a uma área específica com as receitas subjacentes (`just --list`):

| Receita | Banco? | Cobre |
|--------|-----------|--------|
| `just test-rust` | não | Todo o workspace Rust: unidades do motor, fluxo completo emitir→apresentar→validar (EdDSA + ES256), modos de falha de revogação / replay / adulteração, E2E do verificador via relay, interop emissor↔verificador e a ida e volta UniFFI da carteira (`cargo test -p wallet-ffi`). |
...
| `just test-wallet` | não | Compila a biblioteca FFI do host e então roda a suíte da carteira Kotlin + o oráculo de conformidade entre linguagens sobre o motor UniFFI. |
